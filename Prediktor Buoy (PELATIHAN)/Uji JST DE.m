...
%%% UJI IT2FIS
%% Load Fuzzy
it2fis(1).model=readt2fis('Fuzzy2Suhu.t2fis');
it2fis(2).model=readt2fis('Fuzzy2Suhu.t2fis');
it2fis(3).model=readt2fis('Fuzzy2Suhu.t2fis');
it2fis(4).model=readt2fis('Fuzzy2Kelembaban.t2fis');
% it2fis(5).model=readt2fis('Fuzzy2CH.t2fis');
it2fis(6).model=readt2fis('Fuzzy2KecepatanAngin.t2fis');
it2fis(7).model=readt2fis('Fuzzy2ArahAngin.t2fis');
...
hold on
plot(net(nvar).keluaran,'r')
plot(it2fis(nvar).keluaran,'g')
hold off
grid on
title(strcat(['(UJI) Grafik Keluaran Hybri vs JST vs IT2FIS dengan nilai RMSE = ',...
num2str(hybrid(nvar).rmse), ' MAPE= ',...
num2str(hybrid(nvar).mape),'%']))
xlabel('Data Ke-')
ylabel(Variabel)
legend('Keluaran Hybrid','JST','IT2FIS','Location','Best')
end