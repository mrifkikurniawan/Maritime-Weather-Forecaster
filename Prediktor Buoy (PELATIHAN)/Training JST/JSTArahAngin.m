...
JumlahVarInput=7;               %Jumlah Variabel Input
...
datamaksWD=max(max(Input));
dataminWD=min(min(Input));

for xin=1:m
    for yin=1:n
        data_normalisasiinput(xin,yin)=(Input(xin,yin)-dataminWD)/(datamaksWD-dataminWD);
    end
end
...
for xtar=1:o
    for ytar=1:p
        data_normalisasitarget(xtar,ytar)=(Target(xtar,ytar)-dataminWD)/(datamaksWD-dataminWD); 
    end
end
...
AF2='purelin';                   %Fungsi Aktivasi output
...
for epoch=[1000 2000 4000]       %Variasi Jumlah Epoch
   ...
   % Proses prediksi
    ...
    % Denormalisasi
    HasilKeluaran = ((hasil_latih)*(datamaksWD-dataminWD))+dataminWD;
    TargetAsli=((Target)*(datamaksWD-dataminWD))+dataminWD;
    ...
    if fullnetoutput(pop).rmse<bestnetWD.rmse
       bestnetWD=fullnetoutput(pop);
    end
...
%% SIMULASI BEST NET
bestnetWD.output = sim(bestnetWD.net{1,1},Input);
    
% Denormalisasi
bestnetWD.output = ((bestnetWD.output)*(datamaksWD-dataminWD))+dataminWD;
TargetBest=((data_normalisasitarget)*(datamaksWD-dataminWD))+dataminWD;
%Pembulatan hasil
bestnetWD.output = round(bestnetWD.output,1);

%Penentuan Data Training
train=round(RasioTraining/100*r,0);
    for v=1:train
        outputrain(:,v)=bestnetWD.output(:,v);    
    end
    ...
    for v2=train+1:n
        outputval(:,v2-train)=bestnetWD.output(:,v2);
    end
    for l2=train+1:n
