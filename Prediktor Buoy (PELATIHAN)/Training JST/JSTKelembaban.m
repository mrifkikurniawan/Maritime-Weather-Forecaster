...
Variabel='Kelembaban Rata-Rata';

Sheet=4;                        %Letak sheet database
...
datamaksRH=max(max(Input));
dataminRH=min(min(Input));

for xin=1:m
    for yin=1:n
        data_normalisasiinput(xin,yin)=(Input(xin,yin)-dataminRH)/(datamaksRH-dataminRH);
    end
end
...
for xtar=1:o
    for ytar=1:p
        data_normalisasitarget(xtar,ytar)=(Target(xtar,ytar)-dataminRH)/(datamaksRH-dataminRH); 
    end
end
...
for epoch=[1000 2000 4000]       %Variasi Jumlah Epoch
    ...
    % Denormalisasi
    HasilKeluaran = ((hasil_latih)*(datamaksRH-dataminRH))+dataminRH;
    TargetAsli=((Target)*(datamaksRH-dataminRH))+dataminRH;
    ...
    if fullnetoutput(pop).rmse<bestnetRH.rmse
       bestnetRH=fullnetoutput(pop);
    end
...
%% SIMULASI BEST NET
bestnetRH.output = sim(bestnetRH.net{1,1},Input);
    
% Denormalisasi
bestnetRH.output = ((bestnetRH.output)*(datamaksRH-dataminRH))+dataminRH;
TargetBest=((data_normalisasitarget)*(datamaksRH-dataminRH))+dataminRH;
%Pembulatan hasil
bestnetRH.output = round(bestnetRH.output,1);

%Penentuan Data Training
train=round(RasioTraining/100*r,0);
    for v=1:train
        outputrain(:,v)=bestnetRH.output(:,v);    
    end
    ...
    for v2=train+1:n
        outputval(:,v2-train)=bestnetRH.output(:,v2);
    end
    for l2=train+1:n
