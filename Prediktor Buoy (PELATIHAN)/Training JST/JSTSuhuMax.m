...
Lokasi='E:\Data Kaka\Kuliah\Teknik Fisika\Tugas Akhir\Tugas Akhir Kaka\Data-Data\Data Siap\Dengan Uji Outlier\TRAINING & VALIDASI.xlsx';
Variabel='Suhu Maksimum';
Sheet=3;                        %Letak sheet pada excel
...
datamaksTmax=max(max(Input));
dataminTmax=min(min(Input));

for xin=1:m
    for yin=1:n
        data_normalisasiinput(xin,yin)=(Input(xin,yin)-dataminTmax)/(datamaksTmax-dataminTmax);
    end
end
...
for xtar=1:o
    for ytar=1:p
        data_normalisasitarget(xtar,ytar)=(Target(xtar,ytar)-dataminTmax)/(datamaksTmax-dataminTmax); 
    end
end
...
for epoch=[1000 2000 4000]       %Variasi Jumlah Epoch
    ...
    % Denormalisasi
    HasilKeluaran = ((hasil_latih)*(datamaksTmax-dataminTmax))+dataminTmax;
    TargetAsli=((Target)*(datamaksTmax-dataminTmax))+dataminTmax;
    ...
    if fullnetoutput(pop).rmse<bestnetTmax.rmse
       bestnetTmax=fullnetoutput(pop);
    end
...
%% SIMULASI BEST NET
bestnetTmax.output = sim(bestnetTmax.net{1,1},Input);
    
% Denormalisasi
bestnetTmax.output = ((bestnetTmax.output)*(datamaksTmax-dataminTmax))+dataminTmax;
TargetBest=((data_normalisasitarget)*(datamaksTmax-dataminTmax))+dataminTmax;
%Pembulatan hasil
bestnetTmax.output = round(bestnetTmax.output,1);

%Penentuan Data Training
train=round(RasioTraining/100*r,0);
    for v=1:train
        outputrain(:,v)=bestnetTmax.output(:,v);    
    end
    ...
    for v2=train+1:n
        outputval(:,v2-train)=bestnetTmax.output(:,v2);
    end
    for l2=train+1:n
