...
Sheet=5;                        %Letak sheet database
DataIO=xlsread(Lokasi,Sheet);   %Proses pembacaan data excel
JumlahVarInput=6;               %Jumlah Variabel Input
...
datamaksCH=max(max(Input));
dataminCH=min(min(Input));

for xin=1:m
    for yin=1:n
        data_normalisasiinput(xin,yin)=(Input(xin,yin)-dataminCH)/(datamaksCH-dataminCH);
    end
end
...
for xtar=1:o
    for ytar=1:p
        data_normalisasitarget(xtar,ytar)=(Target(xtar,ytar)-dataminCH)/(datamaksCH-dataminCH); 
    end
end
...
for epoch=[1000 2000 4000]       %Variasi Jumlah Epoch
   ...
   % Proses prediksi
    ...
    % Denormalisasi
    HasilKeluaran = ((hasil_latih)*(datamaksCH-dataminCH))+dataminCH;
    TargetAsli=((Target)*(datamaksCH-dataminCH))+dataminCH;
    ...
    if fullnetoutput(pop).rmse<bestnetCH.rmse
       bestnetCH=fullnetoutput(pop);
    end
...
%% SIMULASI BEST NET
bestnetCH.output = sim(bestnetCH.net{1,1},Input);
    
% Denormalisasi
bestnetCH.output = ((bestnetCH.output)*(datamaksCH-dataminCH))+dataminCH;
TargetBest=((data_normalisasitarget)*(datamaksCH-dataminCH))+dataminCH;
%Pembulatan hasil
bestnetCH.output = round(bestnetCH.output,1);

%Penentuan Data Training
train=round(RasioTraining/100*r,0);
    for v=1:train
        outputrain(:,v)=bestnetCH.output(:,v);    
    end
    ...
    for v2=train+1:n
        outputval(:,v2-train)=bestnetCH.output(:,v2);
    end
    for l2=train+1:n
